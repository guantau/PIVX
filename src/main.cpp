...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight <= 5000000 && nHeight > 4000000) {
        nSubsidy = 7.8125 * COIN;
    } else if (nHeight <= 6000000 && nHeight > 5000000) {
        nSubsidy = 3.90625 * COIN;
    } else if (nHeight <= 7000000 && nHeight > 6000000) {
        nSubsidy = 1.953215 * COIN;
    } else if (nHeight <= 8000000 && nHeight > 7000000) {
        nSubsidy = 0.9766075 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
    return nSubsidy;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Do not allow POS block in POW period
